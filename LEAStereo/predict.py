...
def test_kitti(leftname, rightname, savename):
    ...
    color_savename = savename.split('.png')
    color_savename = color_savename[0] + '_color.png'
    print(color_savename)

    # Save color
    disp_np_ours_color = disp2rgb(temp / 192) * 256
    cv2.imwrite(color_savename, disp_np_ours_color)
...
if __name__ == "__main__":
    ...
    for index in range(len(filelist)):
        ...
        if opt.kitti2015:
            ...
            leftname = file_path + 'image_2/' + current_file[0: len(current_file)]
            #rightname = file_path + 'image_3/' + current_file[0: len(current_file) - 1]
            rightname = file_path + 'image_3/' + current_file[0: len(current_file)]
            #savename = opt.save_path + current_file[0: len(current_file) - 1]
            savename = opt.save_path + current_file[0: len(current_file)]
